...
<head>
  ...
  <script>
    ...
    window.TOCCA_FACE_ENROLL = <?php echo tocca_config('voter_face_enroll') ? 'true' : 'false'; ?>;
...
<div class="modal fade voter-modal" id="faceEnrollModal" tabindex="-1" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title">Face check</h5>
      </div>
      <div class="modal-body">
        <p class="mb-2">Look at the camera. One face must be in the frame. This photo is saved with your mobile number, then you set a 4-digit access code.</p>
        <video id="faceEnrollVideo" class="w-100 rounded bg-dark" autoplay playsinline muted></video>
        <canvas id="faceEnrollCanvas" class="d-none"></canvas>
        <div id="faceEnrollMessage" class="mt-2 small text-danger"></div>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-outline-secondary" id="faceEnrollCancelBtn">Cancel</button>
        <button type="button" class="btn btn-primary" id="faceEnrollCaptureBtn">Save face and continue</button>
      </div>
    </div>
  </div>
</div>
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    try {
      ...
      if (statusData.status === "new") {
        ...
        showMobileLookupResult({
          title: "New Voter",
          message: window.TOCCA_FACE_ENROLL
            ? `<strong>${mobile}</strong> is not registered yet. Next, a face check with your camera, then a 4-digit access code. No text message is sent.`
            : `<strong>${mobile}</strong> is not registered yet. Next, verify this number with reCAPTCHA and the OTP sent to your phone.`,
          tone: "primary",
          icon: "fa-user-plus",
  ...
  const openFaceEnrollModal = async (mobile) => {
    if (!/^09\d{9}$/.test(mobile)) return;
    lastCheckedMobile = mobile;
    localStorage.setItem("otp_mobile", mobile);
    localStorage.setItem("voter_mobile", mobile);
    const message = document.getElementById("faceEnrollMessage");
    ...
    try {
      ...
      faceEnrollStream = await navigator.mediaDevices.getUserMedia({
        video: { facingMode: "user" },
        audio: false
      });
      if (video) video.srcObject = faceEnrollStream;
    } catch (error) {
      if (message) message.textContent = "The camera could not start. Allow camera access and try again.";
    }
  };
  document.getElementById("faceEnrollCancelBtn")?.addEventListener("click", () => {
    stopFaceEnrollCamera();
    bootstrap.Modal.getOrCreateInstance(document.getElementById("faceEnrollModal")).hide();
  });
  document.getElementById("faceEnrollModal")?.addEventListener("hidden.bs.modal", stopFaceEnrollCamera);
  document.getElementById("faceEnrollCaptureBtn")?.addEventListener("click", async () => {
    const message = document.getElementById("faceEnrollMessage");
    const video = document.getElementById("faceEnrollVideo");
    const canvas = document.getElementById("faceEnrollCanvas");
    const button = document.getElementById("faceEnrollCaptureBtn");
    const mobile = resolveCheckedMobile();
    if (!video || !canvas || video.videoWidth < 2) {
      if (message) message.textContent = "The camera is not ready yet.";
      return;
    }
    if (message) message.textContent = "";
    ...
    try {
      if (!("FaceDetector" in window)) {
        throw new Error("This browser cannot check that a face is in the photo. Use Chrome on this computer.");
      }
      const detector = new FaceDetector({ fastMode: true, maxDetectedFaces: 2 });
      const faces = await detector.detect(video);
      if (faces.length !== 1) {
        throw new Error(faces.length === 0 ? "No face found. Look at the camera and try again." : "Only one face can be in the frame.");
      }
      canvas.width = video.videoWidth;
      canvas.height = video.videoHeight;
      canvas.getContext("2d").drawImage(video, 0, 0);
      const image = canvas.toDataURL("image/jpeg", 0.85);
      const response = await fetch("enroll_voter_face.php", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        credentials: "same-origin",
        body: JSON.stringify({ mobile_number: mobile, image })
      });
      const data = await response.json();
      if (!response.ok || data.status !== "success") {
      ...
      if (message) message.textContent = error.message || "Could not save the photo.";
    } finally {
  ...
  const proceedAsNewVoter = () => {
    const mobile = resolveCheckedMobile();
    if (window.TOCCA_FACE_ENROLL) {
      openFaceEnrollModal(mobile);
      return;
    }
