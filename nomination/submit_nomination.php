...
// Gallery (multi-file images + videos) — uploaded along with the nomination,
// later copied into tbl_choice_media when the nomination is approved.
...
define('NOM_MEDIA_MAX_FILES',      8);                  // total per nomination
...
set_exception_handler(function(Throwable $e){
  ...
  json_err('Unable to submit your nomination. Please try again later.', 500);
});
...
/* ---------- Read establishment_type_id (required in form) ---------- */
$establishment_type_id = isset($_POST['establishment_type_id']) ? (int)$_POST['establishment_type_id'] : 0;
if ($establishment_type_id <= 0) {
  json_err('Please select an establishment type.', 422);
}
if (!et_type_belongs_to_event($conn, $establishment_type_id, $event_id)) {
  json_err('Please select a valid establishment type for this event.', 422);
}
...
/* ---------- Load active fields ---------- */
$fieldDefs = []; // id => ['id','name','label','type','is_required']
...
/**
 * Save the multi-file gallery uploaded with the nomination. Returns the
...
function save_nomination_media(mysqli $conn, int $nominationId): int {
  ...
  if (!$insert) throw new RuntimeException('Prepare nomination media insert failed.');
...
if (!et_awards_match_establishment_type($conn, $selected_awards, $establishment_type_id, $event_id)) {
  json_err('One or more selected awards are not allowed for your establishment type.', 422);
}
...
// Strict phone validation for any active nomination field that looks like
// mobile/contact/phone (required or optional).
foreach ($fieldDefs as $def) {
  $nameLabel = strtolower(trim((string)($def['name'] ?? '') . ' ' . (string)($def['label'] ?? '')));
  if (!preg_match('/(mobile|phone|contact|telephone)/i', $nameLabel)) continue;
  $rawVal = posted_value_for_field($def);
  if (is_array($rawVal)) continue;
  $raw = trim((string)($rawVal ?? ''));
  if ($raw === '') continue; // optional field left blank
...
/* ---------- Schema detection for establishment_type_id on nominations ---------- */
...
try {
  // 1) Create nomination row (include establishment_type_id if column exists)
  if ($hasNomEstTypeCol) {
  ...
  if (!$ins->execute()) throw new RuntimeException('Insert nomination failed: '.$ins->error);
  ...
  foreach ($fieldDefs as $def) {
    ...
    if (is_array($val)) {
      ...
      $nameLabel = ($def['name'] ?? '') . ' ' . ($def['label'] ?? '');
      if (preg_match('/mayor/i', $nameLabel) && $type !== 'file') {
        $answer = normalize_mayors_permit($answer) ?? $answer;
      }
      if (preg_match('/(mobile|phone|contact|telephone)/i', $nameLabel)) {
  ...
  /* If nominations table does NOT have establishment_type_id,
  ...
  $storage_note = $hasNomEstTypeCol
    ? 'Saved to tbl_nominations.establishment_type_id'
    : ($est_type_field_id !== null ? 'Saved to tbl_nomination_answers (field: establishment_type_id)' : 'Not saved (add column or field to persist)');
