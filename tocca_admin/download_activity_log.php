...
$sqlChoice = "
    SELECT q.question_name, 'Dropdown' AS answer_type,
           CASE
             WHEN LOWER(TRIM(COALESCE(q.answer_fields, ''))) = 'meryenda'
                  AND pf.freetext IS NOT NULL AND TRIM(pf.freetext) <> ''
             THEN CONCAT({$answerSql}, ' — ', pf.freetext)
             ELSE {$answerSql}
           END AS answer,
           DATE(pc.vote_at) AS vote_date
    ...
    LEFT JOIN tbl_poll_freetext pf ON pf.voters_id = pc.voters_id AND pf.question_id = pc.question_id
    {$entryJoin}
    WHERE pc.voters_id = ?
";
