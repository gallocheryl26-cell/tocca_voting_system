...
if (isset($conn) && $conn instanceof mysqli) {
    ...
    try {
        ...
        $GLOBALS['tocca_db_error'] = 'Database unavailable. Start MySQL in XAMPP, then try again.';
        $allowSoftFail = defined('TOCCA_ALLOW_DB_FAIL') && TOCCA_ALLOW_DB_FAIL;
        if ($allowSoftFail) {
            $conn = null;
            $GLOBALS['conn'] = null;
        } else {
            ...
            if (!headers_sent() && (php_sapi_name() === 'cli' || str_contains($_SERVER['REQUEST_URI'] ?? '', '.php'))) {
                ...
                if ($wantsJson) {
                    ...
                    echo json_encode(['success' => false, 'message' => $GLOBALS['tocca_db_error']]);
                    exit;
                }
            }
