...
if (!function_exists('results_formula_unique_cast_votes')) {
    ...
    function results_formula_unique_cast_votes(mysqli $conn, int $eventId): array
    {
        ...
        $st = $conn->prepare(
            "SELECT c.category_name, pf.freetext
             FROM tbl_poll_freetext pf
             INNER JOIN tbl_questions q ON pf.question_id = q.question_id
             INNER JOIN tbl_categories c ON q.category_id = c.category_id
             WHERE c.event_id = ? AND q.answer_fields = 'meryenda'
               AND NOT EXISTS (
                 SELECT 1 FROM tbl_poll_choice pc
                 WHERE pc.voters_id = pf.voters_id AND pc.question_id = pf.question_id
               )"
        ...
        if ($st) {
            ...
            while ($row = $res->fetch_assoc()) {
                $cat = (string) ($row['category_name'] ?? '');
                $parts = freetext_vote_parse((string) ($row['freetext'] ?? ''));
                $label = freetext_vote_title_case((string) ($parts['title'] ?? ''));
                if ($cat === '' || $label === '') {
                    continue;
                }
                $key = $cat . "\n" . freetext_vote_part_key($label);
                if (!isset($customCounts[$key])) {
                    $customCounts[$key] = ['category' => $cat, 'label' => $label, 'vote_count' => 0];
                }
                $customCounts[$key]['vote_count']++;
            }
            $st->close();
            foreach ($customCounts as $custom) {
                $cat = $custom['category'];
                $count = (int) $custom['vote_count'];
                $prev = $out['top_by_category'][$cat]['vote_count'] ?? 0;
                if ($count > $prev) {
                    $out['top_by_category'][$cat] = [
                        'choice_name' => $custom['label'],
                        'vote_count' => $count,
                    ];
                }
            }
    ...
    function results_formula_fetch_for_award(mysqli $conn, int $event_id, int $question_id): array
    {
        ...
        if ($answerFields === 'meryenda') {
            ...
            if ($st) {
                ...
                while ($row = $res->fetch_assoc()) {
                    $parts = freetext_vote_parse((string) ($row['freetext'] ?? ''));
                    $product = freetext_vote_title_case((string) ($parts['title'] ?? ''));
                    if ($product === '') {
                        continue;
                    }
                    $customRows[] = [
                ...
                $st->close();
                foreach (freetext_vote_merge_rows_single($customRows) as $merged) {
                    $rows[] = $merged;
                }
            }
        }
